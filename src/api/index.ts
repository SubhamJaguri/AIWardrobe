...
export async function fetchModels(gender:string) {
  ...
  const response = await axios.get(`${API_BASE_URL}/get_selected_models?gender=${gender}`, {
     headers,
  });
  return response.data;
 }
...
export const deleteGarment = async (garment_id: string) => {
 ...
 export const tryOnGarments = async (modelId: string, topsId: string, bottomsId?: string): Promise<TryOnResponse> => {
  const data ={
     "garments": {
       "tops":topsId,
       "bottoms": bottomsId,
